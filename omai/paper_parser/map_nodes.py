...
MAP_USER_INSTRUCTIONS = (
    ...
    "(full/exact) LBTE solver. A MEASURED thermal conductivity, or a computed "
    "one reported WITHOUT a resolvable solver/method, maps to the "
    "method-neutral node 'ThermalConductivity' (the physical observable), NOT "
    "to a solver-labeled node and NOT to [contribution=total] (which is only "
    "for an explicit lattice+electronic sum). Use a labeled node only when the "
    "claim states its computational route.\n"
