"""PhysLean export (Tier 1, EXPERIMENTAL): the map's dimensional layer as Lean.
...
is generated against PhysLean. The dimensional identities are verified in
the test suite (the property `decide` would check); COMPILING the file against
PhysLean is the honest remaining step, so this tier is marked experimental.
...
def _render_lean(nodes: dict, lemmas: list, node_dims: dict) -> str:
    ...
    lines = [
        "/-",
        "  OpenMaterials: the map's dimensional layer as Lean (EXPERIMENTAL).",
        "",
        "  Generated by omai.physlean_export from map version " + ver + ".",
        "  EXPERIMENTAL: dimensional identities are checked in Python; this file is",
        "  not yet compiled against PhysLean (see physlean/README).",
