...
# ---------------------------------------------------------------------------
# Physlib crosswalk: static, reviewed mapping from map node ids to the Lean 4
# declarations (physlib) that formalize the same concept. Authored by hand
# against a pinned physlib rev (docs/data/physlib_crosswalk.json), never
# regenerated by map_data: formal alignment is reviewed like vocabulary.
# ---------------------------------------------------------------------------
_CROSSWALK = _DOCS / "data" / "physlib_crosswalk.json"
_crosswalk_nodes: dict[str, list[dict]] | None = None


def physlib_for(node_id: str) -> list[dict]:
    """The physlib declarations formalizing a map node. Empty list means an
    honest coverage gap (physlib is young), not an error."""
    global _crosswalk_nodes
    if _crosswalk_nodes is None:
        _crosswalk_nodes = (json.loads(_CROSSWALK.read_text())["nodes"]
                            if _CROSSWALK.exists() else {})
    return _crosswalk_nodes.get(node_id, [])
...
def resolve(label: str, semantics: list[dict], limit: int = 8) -> list[dict]:
    ...
    A node hit gains a "physlib" field when the reviewed crosswalk maps
    it to Lean 4 declarations (docs/data/physlib_crosswalk.json).
    """
    q = normalize(label)
    qt = _tokens(label)
    hits = []
    for e in entries_iter(semantics):
        ...
        if score:
            hit = {"id": e["id"], "uid": e["uid"], "kind": e["kind"],
                   "score": round(score, 3), "why": why}
            if e["kind"] == "node":
                physlib = physlib_for(e["id"])
                if physlib:
                    hit["physlib"] = physlib
            hits.append(hit)
