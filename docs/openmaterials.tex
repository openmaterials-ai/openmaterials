...
\paragraph{Simulation record (a light, recipe-identified, URL-first run).} The
...
\texttt{recipe}: the map node when one is known (with a content-uid pin), else a
...
recipe alone, content-addressed by the sha256 of its canonical JSON with the same
...
the recipe names a map node, lights that node on the map, so an experiment is a
link, no server, that carries the recipe and its receipts together. The get is
...
and uid when the recipe names one (a stale pin is a mismatch, never a silent
...
the recipe and its identity, the platform holds the artifacts, referenced by
