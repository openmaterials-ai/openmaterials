...
An experimental first tier exists (2026-07-12): \texttt{omai.physlean\_export} generates
        a Lean file in which every node is a PhysLean \texttt{Dimension} and every
        dimension-proven edge is a theorem closed by \texttt{decide}, regenerated with the map.
        The four bases PhysLean shares with the map (length, time, mass, temperature) map
        directly; nodes carrying amount-of-substance, current, or luminous intensity are
        omitted, since PhysLean's \texttt{Dimension} has no base for them. The dimensional
        identities are checked in the test suite (the property \texttt{decide} would verify);
        compiling the file against PhysLean, and the approximation-error theorems, remain
        future work.
