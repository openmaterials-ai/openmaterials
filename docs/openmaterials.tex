...
\title{\textbf{openmaterials.ai}\\[0.3em]
       \large\normalfont Git for science: a versioned map of physics}
\author{Giuseppe Barbalinardo\\\small University of California, Davis
        \and Davide Donadio\\\small University of California, Davis}
\date{\today}
...
it, its lineage, its units and gauge, and the codes that represent it. This
...
representation layer, 27 ingested representations (kaldo, phono3py, phonopy,
pymatgen, ShengBTE, Quantum ESPRESSO, VASP, LAMMPS, GPUMD, the machine-learned
...
transport (46 quantities, 47 operators), the Quantum ESPRESSO DFT ground
...
a first materials-diffusion subgraph, and a thermodynamic-identities domain that
closes the map's formulas together; the unified map of
98 quantities and 94 operators is content-addressed in the log-first store
(Part~\ref{part:kernel}), grown past its frozen genesis version through the
contribution gates, and the whole is held together by a suite of 1327
passing tests.
...
and check, the same way it already ingests a code.
...
\textbf{Evidence} is values on nodes. It comes from \textbf{instances}: one run of a
...
onto them); instances and spectra are a distinct evidence layer that consumers
load against node uids and attach locally, never carried on the map page itself.
...
and conventions. A single run of that code is an \textbf{instance}, and the kappa it
outputs is a \textbf{value on the kappa node}. A growth process such as CVD is an
...
The map is a curated reference graph. Today it holds 98 nodes (95 observable
and hidden quantities plus 3 promoted parameters) and 242 edges. Nodes are
...
\begin{itemize}[leftmargin=2em]
  \item The code parser imports a codebase as a representation: it maps the code's
  ...
  an instance (values).
  \item The paper parser turns a paper's formulas into edges (structure) and its
  measured or computed numbers into values on nodes; its first version
  (\texttt{omai/paper\_parser}, extracting values as gated evidence) is
  implemented, detailed under the parser contract below.
  \item The run parser reads a concrete run and records its inputs (from the input
...
contracts below are implemented (Part~\ref{part:kernel}); the parsers remain the
eventual on-ramp, each its own spec, plan, and build cycle.
...
\paragraph{Value record (an instance).} The implemented schema, shown as a real
committed instance, verbatim
...
\texttt{variable} always names a node, because a value belongs to a quantity;
at bundle time each record is additionally pinned to that node's content uid
(\texttt{node\_uid}). The material is deliberately NOT part of the map's
...
against a monotonic axis. A spectrum record is the value record with the scalar
replaced by an axis and an array, attached to the same nodes and pinned the same
...
\paragraph{The paper parser (P1), implemented.} The first parser now exists:
...
reported value with a verbatim, page-located quote (a citations-enabled model
pass); MAP aligns each detected value against the node catalog via structured
outputs; VALIDATE runs deterministically over the kernel, killing any claim whose
quote is not found verbatim in the document (the hallucination gate) and flagging
values that duplicate a committed instance; REVIEW is an adversarial model pass;
and PROPOSE emits a proposal file that a human confirms with \texttt{--apply}
before any instance is written. The pipeline never prints or logs the API key.
It is golden-evaluated against this very document: on the eight expected values it
recovers seven (recall 0.875), no hallucinated quote survives validation, and
every recovered known is correctly flagged as a duplicate of its committed
instance.
...
Today's map is one hundred three change records past v1, its frozen genesis version:
95 typed quantities plus 3 promoted
parameters, the symbolic formula on every relational edge, 27 representations
mapped (kaldo 32 variables, phono3py 31, phonopy 22, pymatgen 22, ShengBTE 20,
mp-api 13, QE 13, LAMMPS 12, VASP 9, GPUMD 8, pycalphad 7, and a further sixteen
codes and mat-* / chem-* skills at five or fewer each), and real instances
...
Part~\ref{part:kernel} documents all three. The parsers are the eventual
automated on-ramp, not the day-one one, and remain the next build after the
store and index.
...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.4em]
  ...
  \item \textbf{Identity is content, not history.} A operator state is identified by its
        ...
        gauge class plus its labels. Provenance is recorded lineage metadata,
        never identity: production routes are recorded but a route never re-mints
...
Each adapter is a representation functor over the operator DAG. Eight adapters
are now ingested, in three tiers. The four BTE / harmonic codes that consume
force constants:
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      \item \texttt{operator/nodes.py}: the 46 spaces as \texttt{ObservableSpace} or
            \texttt{HiddenSpace} instances with their fields.
      \item \texttt{operator/edges.py}: the 47 operators with sympy formulas
...
real on the 51-node map: composable dimensions with a dimensional gate, content
identity by hashing, a log-first versioned store with a tamper-evident hash
chain, a frozen genesis, and the source index. After the kernel landed, the
...
later propose. On the current unified map the gate classifies the 53 edges as
\textbf{21 ok}, \textbf{2 documented-schematic violations}, and \textbf{30
skipped} (edges whose formula is not a closed-form \texttt{Eq} the gate can
check, such as implicit solves and procedural fits). The two violations are
...
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  ...
  \item \textbf{gauge\_class} is one of \texttt{observable},
        \texttt{hidden/scaffolding}, or \texttt{hidden/approximation}, plus the
        declared \texttt{gauge\_group} for hidden spaces.
...
to a node. Each instance pins to an exact element by node uid, so the genesis
migration re-pins the harvested instances to element hashes mechanically. A value
...
contribution gates and sync tool (P4) are built. P4 added the six gates
(registry, identity, reachability, connectivity, gauge, and dimensional) that a
contribution passes through \texttt{Store.propose} before any record lands, and
the \texttt{python -m omai.sync} tool that diffs the Python operator layer
against \texttt{map/current} and proposes change records (dry by default, a
gated apply for additions and metadata edits, re-mints always routed through an
explicit supersede). P5 is done in its minimal honest form: the map page shows
...
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  \item \textbf{The parsers}, the automated on-ramp (code, paper, run), each its
        own spec, plan, and build cycle.
...
\texttt{b505d9421136e70bfa21d89dd6f81ba6a765c0d00b4f7e6226220ed5743a111e}
(2026-07-10, 203 records), one hundred three change records past the frozen genesis hash
\texttt{e6e8044e92039696417b53b220b0f3f10559a286b0eaabbe7ea4167ff510f6cd}
(2026-07-07, the untouched 100-record prefix). The live numbers:

\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 98 nodes (95 observable and hidden quantities plus 3
        promoted parameters), 242 rendered links, 94 deduplicated operators,
        rendered in fifteen physics tiers (Sources, Harmonic, Thermodynamics,
        ...
        transport, Diffusion, Thermoelectric). The newest tier (Thermoelectric)
        and its power-factor and ZT nodes landed with the thermodynamic-identities
        domain that closes the map's formulas together; the five before it
        (Stability, Thermochemistry, Quasi-harmonic, Molecular, Electronic
        transport) landed from the AtomisticSkills scans. The
  ...
  \item \textbf{Representations.} 27 codes mapped, with per-code variable coverage:
        kaldo 32, phono3py 31, phonopy 22, pymatgen 22, ShengBTE 20, mp-api 13,
        Quantum ESPRESSO 13, LAMMPS 12, VASP 9, GPUMD 8, pycalphad 7, matgl 5,
        mat-elasticity 5, amset 5, ORCA 5, pymatgen-analysis-diffusion 5,
        fairchem 4, MACE 4, OpenMM 4, ASE 3, mat-diffusion-analysis 2, smol 2,
        mat-equation-of-state 1, mat-surface-adsorption 1, rxn-network 1,
        diffcsp 1, mattergen 1.
  \item \textbf{Instances.} 45 recorded values, 43 simulation and 2 measurement.
        ...
        frequency). The two measurements are the Glassbrenner--Slack 1964
        steady-state thermal conductivities of bulk single-crystal silicon
        ($156\,\mathrm{W/(m\,K)}$) and germanium ($60.2\,\mathrm{W/(m\,K)}$) at
        $300\,\mathrm{K}$ and natural isotopic abundance.
  \item \textbf{Dimensional gate.} Of the 100 operator edges the gate walks, 37
        are ok, 2 are documented-schematic violations
        (\texttt{compute\_gruneisen} and \texttt{compute\_phase\_space\_3phonon},
        kept schematic by choice), and 61 are skipped (not closed-form
        \texttt{Eq} edges the gate can check, most of them the opaque solver
        edges of the newer domains). The two ground-state derivatives
        ...
        $C_P - C_V$ relation, the power factor, and $ZT$).
  \item \textbf{Tests.} The suite is 1327 passing.
\end{itemize}
...
project's GitHub LICENSE for GPUMD, MatterGen, DiffCSP, and OpenMM). VASP and
...
Each code rail on the map is cited below. References were taken from each code's
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.25em]
  ...
  \item \textbf{pymatgen-analysis-diffusion}: Z. Deng, Z. Zhu, I.-H. Chu,
        S. P. Ong, \emph{Data-driven first-principles methods for the study of
        ionic conductivity}, Chem. Mater. \textbf{29}, 281 (2017).
        doi:10.1021/acs.chemmater.6b02648.
  \item \textbf{rxn-network}: M. J. McDermott, S. S. Dwaraknath, K. A. Persson,
        \emph{A graph-based network for predicting inorganic reaction pathways},
        Nat. Commun. \textbf{12}, 3097 (2021). doi:10.1038/s41467-021-23339-x.
  ...
  \item \textbf{ASE}: A. H. Larsen, J. J. Mortensen, J. Blomqvist, et al.,
  ...
  \item \textbf{mat-elasticity, mat-diffusion-analysis, mat-equation-of-state,
        ...
        Research} (2025), arXiv:2605.24002. doi:10.48550/arXiv.2605.24002.
\end{itemize}
...
\begin{enumerate}[leftmargin=2em, itemsep=0.2em]
  ...
  \item \textbf{The parsers}, the eventual automated on-ramp (code, paper, run),
        each its own spec, plan, and build cycle.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Added \texttt{KM\_PER\_S = Unit(...)}} to \texttt{units.py} with
        \texttt{to\_operator = 10.0} ($1\,\mathrm{km/s} = 10\,\mathrm{\AA\cdot
        THz}$). This kind of one-line addition to \texttt{units.py} is routine:
  ...
  \item \textbf{$\kappa_{\mathrm{CONV}} \neq$ direct\_inverse algorithm, but $=$
        ...
        \texttt{discretization\_choices.linear\_solver}.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Adapters.} For every code that produces the new quantity, add a
        ...
        \texttt{omai/<domain>/representation/<code>.py}. If a code does not produce
...
\texttt{docs/data/instances/lgps-activationenergy-atomisticskills-mat-diffusion-analysis-lgps.json}
