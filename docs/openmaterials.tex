...
representation layer, 28 ingested representations (kaldo, phono3py, phonopy,
...
105 quantities and 104 operators is content-addressed in the log-first store
(Part~\ref{part:kernel}), grown past its frozen genesis version through the
contribution gates, and the whole is held together by a suite of 1408
passing tests.
...
One run of a representation     & \textbf{instance} & serialized as a \emph{value record}. \\
...
The map is a curated reference graph. Today it holds 105 nodes (102 observable
and hidden quantities plus 3 promoted parameters) and 255 edges. Nodes are
...
large-scale literature analysis) align to the map through reviewed alignment
files, each unmatched label ranked by its usage frequency in the field. This
...
catalog builder) is classified as a run \emph{condition}, not a value. Such
...
Today's map is one hundred twenty-three change records past v1, its frozen genesis version:
99 typed quantities plus 3 promoted
parameters, the symbolic formula on every relational edge, 28 representations
mapped (kaldo 34 variables, phono3py 31, phonopy 22, pymatgen 22, ShengBTE 20,
mp-api 13, QE 13, LAMMPS 12, VASP 9, GPUMD 8, pycalphad 7, i-PI 4, and a further
sixteen codes and mat-* / chem-* skills at five or fewer each), and real instances
...
improvement engine are built by Da Vinci Labs against that boundary.
...
The first tier is built and verified (2026-07-12): \texttt{omai.physlean\_export} generates
        a Lean file in which every node is a PhysLean \texttt{Dimension} and every
        dimension-proven edge is a theorem stating the output dimension equals the product of
        the input dimensions. The file \emph{compiles} against PhysLean (Lean~4.29.1): 80
        nodes, 11 edge theorems, zero errors. The four bases PhysLean shares with the map
        (length, time, mass, temperature) map directly; nodes carrying amount-of-substance,
        current, or luminous intensity are omitted, since PhysLean's \texttt{Dimension} has no
        base for them. So the dimensional layer type-checks in a proof assistant today. A second tier
        also compiles: the chained thermodynamic identities (the thermoelectric
        figure of merit through the power factor, the constant-pressure heat capacity
        through the molar volume) are proved equal to their expanded forms, and a
        units file anchors the map's SI convention against PhysLean's conversion
        structure. The approximation-error theorems remain future work.
...
\texttt{8aea09f90c24ae29137be4860452b9085d5b1e1a502493bffd8f956a9c382a8f}
(2026-07-12, 223 records), one hundred twenty-three change records past the frozen genesis hash
\texttt{e6e8044e92039696417b53b220b0f3f10559a286b0eaabbe7ea4167ff510f6cd}
(2026-07-07, the untouched 100-record prefix). The live numbers:

\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 105 nodes (102 observable and hidden quantities plus 3
        promoted parameters), 256 rendered links, 104 deduplicated operators,
  ...
  \item \textbf{Representations.} 29 codes mapped, with per-code variable coverage:
        ...
        i-PI 4, fairchem 4, MACE 4, OpenMM 4, ASE 3, mat-diffusion-analysis 2,
        smol 2, mat-equation-of-state 1, mat-surface-adsorption 1, rxn-network 1,
        diffcsp 1, mattergen 1.
  \item \textbf{Instances.} 82 recorded values, 78 simulation and 4 measurement.
        ...
        frequency). Fourteen of the simulation values are the first
        paper-sourced evidence: thermal conductivities and one mass density
        parsed from three published papers, each landed with a verbatim,
        page-located quote (kaldo 2020, the QHGK/Green-Kubo Isaeva 2019, and
        Esfarjani 2011). The two measurements are the Glassbrenner--Slack 1964
  ...
  \item \textbf{Dimensional gate.} Of the 104 operator edges the gate walks, 38
        are ok, 2 are documented-schematic violations
        (\texttt{compute\_gruneisen} and \texttt{compute\_phase\_space\_3phonon},
        kept schematic by choice), and 64 are skipped (not closed-form
        \texttt{Eq} edges the gate can check, most of them the opaque solver
        edges of the newer domains, now including the two PIMD estimator edges).
        The two ground-state derivatives
  ...
  \item \textbf{Tests.} The suite is 1425 passing.
\end{itemize}
