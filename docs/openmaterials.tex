...
representation layer, nine ingested representations (kaldo, phono3py, phonopy,
ShengBTE, Quantum ESPRESSO, ASE, LAMMPS, GPUMD, and one AtomisticSkills
workflow), and a validation engine that executes the graph, composes
...
transport (46 quantities, 47 operators) and a first materials-diffusion
subgraph; the unified map of 51 quantities and 49 operators is
content-addressed and frozen at a genesis version in the log-first store
(Part~\ref{part:kernel}), and the whole is held together by a suite of 817
passing tests.
...
the protocol. The map is small enough today (51 nodes, 49 deduplicated edges) to
...
later propose. On the current unified map the gate classifies the 49 edges as
\textbf{19 ok}, \textbf{2 documented-schematic violations}, and \textbf{28
...
covering \textbf{51 nodes, 49 edges, 100 records}. A repository-state test asserts
that the committed artifact still matches a fresh \texttt{run\_genesis}: it is the
drift alarm between the Python authoring layer and the frozen store, and the
acceptance test replays the log and asserts the materialized view equals the graph
the Python layer generates.
...
(P2), and the store, genesis migration, and \texttt{verify()} (P3) are built. What
remains, in order:
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  \item \textbf{P4: gates on push and the sync tool.} Wire the six validation
        gates (dimensional agreement, reachability, connectivity, gauge
        discipline, vocabulary, and identity-collision) to run on every push, and
        build the sync script that diffs the operator layer against
        \texttt{map/current} and proposes change records for review, so the Python
        modules stay the authoring surface for existing domains until the
        scan-encode pipeline pushes contributions directly.
  \item \textbf{P5: site wiring.} \texttt{docs/data/graph.json} gains its
        \texttt{id} field so the interactive map and \texttt{docs/pipeline.html}
        render from the store's materialized view.
  \item \textbf{The Quantum ESPRESSO DFT ground-state domain}, the first
        contribution pushed through the store, becomes the worked example of a
        full index entry (structure, values, and coverage pinned to element
        hashes at a map version). It follows the \texttt{ingest\_code} discipline
        of Appendix~\ref{app:ingest}: verify the fresh code against a trusted one
        on the same material before any single-code result is trusted.
\end{itemize}
...
The map is at its genesis version, frozen at hash
\texttt{e6e8044e92039696417b53b220b0f3f10559a286b0eaabbe7ea4167ff510f6cd}
(2026-07-07). The live numbers:

\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 51 nodes (48 observable and hidden quantities plus 3
        promoted parameters), 131 rendered links, 49 deduplicated operators. The
  ...
  \item \textbf{Representations.} 9 codes mapped, with per-code variable coverage:
        kaldo 32, phono3py 31, ShengBTE 20, phonopy 17, Quantum ESPRESSO 9,
        LAMMPS 9, GPUMD 8, mat-diffusion-analysis 2, ASE 1.
  \item \textbf{Instances.} 9 recorded values, 7 simulation and 2 measurement.
  ...
  \item \textbf{Dimensional gate.} Of the 49 edges, 19 are ok, 2 are
        documented-schematic violations (\texttt{compute\_gruneisen} and
        \texttt{compute\_phase\_space\_3phonon}, kept schematic by choice), and 28
        are skipped (not closed-form \texttt{Eq} edges the gate can check).
  \item \textbf{Tests.} The suite is 817 passing.
\end{itemize}
...
\begin{enumerate}[leftmargin=2em, itemsep=0.2em]
  \item \textbf{P4}: run the validation gates on every push, and build the sync
        tool that proposes change records from the operator layer for review.
  \item \textbf{P5}: wire the site to render from the store's materialized view
        (\texttt{docs/data/graph.json} gains its \texttt{id} field).
  \item \textbf{The Quantum ESPRESSO DFT ground-state domain}, the first
        contribution pushed through the store and the worked example of a full
        index entry.
