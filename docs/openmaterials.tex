...
and recompiled in CI on every change, its evidence carries pinned conformance
...
records with the same id are the same computation, and any changed condition
mints a different id. The implemented schema, shown as a real committed
instance, verbatim
...
journal) and any number of lineage records, gzip-compressed and
...
asserted link, and \texttt{expected} mirrors the instance's own values. The
...
\begin{itemize}[leftmargin=2em]
  ...
  \item This document as browsable HTML with the PDF as a download:
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Verification.} Project the operator layer into Lean, allowing
        ...
        physlib's five bases, and a generated seven-base extension closes the
        remaining amount-of-substance, current, and luminous-intensity quantities, so
        every one of the map's 114 nodes carries a machine-checked dimension. Tier~2
...
format moves the one version every page and every record cites. The live
numbers:
...
\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  ...
  \item \textbf{Instances.} 91 recorded values, 87 simulation and 4 measurement.
        ...
        $300\,\mathrm{K}$ and natural isotopic abundance. The four newest
  ...
  \item \textbf{Verified layer.} 89 nodes and 12 edge theorems compile against
        ...
        extension covers all 114 nodes, and 13 identity theorems check against
  ...
  \item \textbf{Tests.} The suite is 1727 passing.
\end{itemize}
