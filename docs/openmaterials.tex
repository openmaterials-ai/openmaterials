...
\title{\textbf{openmaterials}\\[0.3em]
...
Today the framework is real and small. It has an operator layer and a
representation layer, ten ingested representations (kaldo, phono3py, phonopy,
ShengBTE, Quantum ESPRESSO, ASE, LAMMPS, GPUMD, and two AtomisticSkills
workflows), and a validation engine that executes the graph, composes
...
state (4 quantities, 4 operators, the first contribution to land through the
protocol gates), the mechanics domain (4 quantities, 4 operators: the elastic
tensor, its Voigt moduli, and the pressure, landed through the same gates),
and a first materials-diffusion subgraph; the unified map of
59 quantities and 57 operators is content-addressed in the log-first store
(Part~\ref{part:kernel}), grown past its frozen genesis version through the
contribution gates, and the whole is held together by a suite of 898
passing tests.
...
The map is a curated reference graph. Today it holds 59 nodes (56 observable
and hidden quantities plus 3 promoted parameters) and 144 edges. Nodes are
...
\paragraph{Value record (an instance).} Sketched:
...
\texttt{confirms} always points at a node, because a value belongs to a quantity. A
...
Today's map is seventeen change records past v1, its frozen genesis version:
56 typed quantities plus 3 promoted
parameters, the symbolic formula on every relational edge, 10 representations
mapped (kaldo 32 variables, phono3py 31, ShengBTE 20, phonopy 17, QE 13,
LAMMPS 11, GPUMD 8, the mat-elasticity skill 3, the mat-diffusion-analysis
skill 2, and ASE 1), and real instances
...
\texttt{docs/pipeline.html} for the interactive view). The
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item \texttt{visualize.py}: emits \texttt{docs/pipeline.html}, an interactive
...
the protocol. The map is small enough today (59 nodes, 57 deduplicated edges) to
verify the genesis migration by hand; after a Quantum ESPRESSO scan it would not
be.
...
\texttt{3e87f5c723d351c17227ad0e1920facda0b07a79e9b085b9a8423327de30b889}
(2026-07-08, 117 records), seventeen change records past the frozen genesis hash
\texttt{e6e8044e92039696417b53b220b0f3f10559a286b0eaabbe7ea4167ff510f6cd}
(2026-07-07, the untouched 100-record prefix). The live numbers:

\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 59 nodes (56 observable and hidden quantities plus 3
        promoted parameters), 144 rendered links, 57 deduplicated operators,
        rendered in nine physics tiers (Sources, Harmonic, Thermodynamics,
        Scattering, Transport, Molecular dynamics, Ground state, Mechanics,
        Diffusion; the Mechanics tier is the newest, between Ground state and
        Diffusion). The
  ...
  \item \textbf{Representations.} 10 codes mapped, with per-code variable coverage:
        kaldo 32, phono3py 31, ShengBTE 20, phonopy 17, Quantum ESPRESSO 13,
        LAMMPS 11, GPUMD 8, mat-elasticity 3, mat-diffusion-analysis 2, ASE 1.
  \item \textbf{Instances.} 13 recorded values, 11 simulation and 2 measurement.
        The two newest are the first mechanics values, the Cu Voigt-Reuss-Hill
        bulk (145.85 GPa) and shear (51.45 GPa) moduli from the AtomisticSkills
        mat-elasticity example; before them came the first Quantum ESPRESSO
        values from the Si
        cross-check (the SCF total energy and the $\Gamma$ optical frequency).
        The two measurements are the Glassbrenner--Slack 1964 steady-state
        thermal conductivities of bulk single-crystal silicon
        ($156\,\mathrm{W/(m\,K)}$) and germanium ($60.2\,\mathrm{W/(m\,K)}$) at
        $300\,\mathrm{K}$ and natural isotopic abundance.
  \item \textbf{Dimensional gate.} Of the 57 edges, 25 are ok, 2 are
        documented-schematic violations (\texttt{compute\_gruneisen} and
        \texttt{compute\_phase\_space\_3phonon}, kept schematic by choice), and 30
        are skipped (not closed-form \texttt{Eq} edges the gate can check). The
        two ground-state derivatives (\texttt{compute\_forces\_hf} and
        \texttt{compute\_stress\_cell}) and all four mechanics edges (the
        elastic tensor as $-\partial\sigma/\partial\varepsilon$, the pressure
        trace, and both Voigt moduli) are proven ok.
  \item \textbf{Tests.} The suite is 898 passing.
\end{itemize}

\begin{center}
\begin{tabular}{lr}
\toprule
Code & Variables mapped \\
\midrule
kaldo                   & 32 \\
phono3py                & 31 \\
ShengBTE                & 20 \\
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \texttt{visualize.py} discovers adapters automatically (any submodule of
        \texttt{representation/} exposing module-level spec instances is picked
        up); regenerate \texttt{docs/pipeline.html} with
        \texttt{python -m omai.thermal\_transport.visualize} and the map data with
        \texttt{python -m omai.map\_data}.
\end{itemize}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Visualization + map data.} \texttt{docs/pipeline.html} regenerates
        from \texttt{python -m omai.thermal\_transport.visualize}, and the unified
        map data (\texttt{docs/data/*.json}) from \texttt{python -m omai.map\_data}.
        No code changes are needed unless you are adding a new pattern that the
        renderer cannot lay out automatically.
...
\texttt{python -m omai.map\_data} rewrites \texttt{docs/data/graph.json},
\texttt{docs/data/instances.json}, and \texttt{docs/data/codes.json}. Commit the
updated HTML alongside any content changes (see the
\texttt{feedback\_regenerate\_dag\_html.md} memory note for the visualize script).
