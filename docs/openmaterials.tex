...
contribution gates, and the whole is held together by a suite of 1489
passing tests.
...
\paragraph{Simulation record (a whole run as evidence).} The value,
configuration, and spectrum records each capture one number, one cell, or one
curve; a \emph{simulation record} captures the whole run that produced them, as
the shareable, re-runnable, fetchable-by-hash unit. It composes from parts that
already exist: a \texttt{recipe} (the asked computation, in map identity: node,
material, conditions, params, the format MaterialsCodeGraph already ships), an
\texttt{execution} block (the code and its version, the per-run container
digest, the runner, wall time, and seeds), an artifact \texttt{manifest}, and
the run's \texttt{results} (the instances it produced, each gaining an optional
\texttt{simulation} backref to the record). The manifest is a list of entries
\texttt{\{path, bytes, sha256, role\}}: a claim about the bytes, never the bytes
themselves, which stay in the owner's object storage (an R2 bucket, a Zenodo
DOI) under the owner's own terms. The record is content-addressed by the sha256
of exactly that claim (recipe, execution, and the manifest \emph{without} URLs),
with the same canonical-JSON and float-rounding discipline the configuration
uid uses. Location is deliberately out of the hash: the URLs live in a
\texttt{mirrors} resolver layer outside the hashed claim, so moving bytes,
renaming a bucket, or adding a mirror never re-mints the record and never
orphans a value that cites it. A verification tool then checks reachability and
checksum match as a dated report, never as a gate: a record whose bytes moved is
stale, not wrong. The cheap validation gates that do apply are the existing
ones: the recipe node resolves against the live map by id and content uid, a
named configuration exists, every manifest entry is well-formed, and every
bundled result instance passes the instance checks and backrefs the record. This
is the same bytes-out-of-git rule the governance already commits to (the map
holds identity and provenance, the owner holds the artifacts), made into a
first-class record kind.
...
\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 114 nodes (111 observable and hidden quantities plus 3
        promoted parameters), 274 rendered links, 109 deduplicated operators,
  ...
  \item \textbf{Representations.} 31 codes mapped, with per-code variable coverage:
        ...
        i-PI 4, fairchem 4, MACE 4, OpenMM 4, ASE 3, materialscodegraph 3,
  ...
  \item \textbf{Instances.} 87 recorded values, 83 simulation and 4 measurement.
