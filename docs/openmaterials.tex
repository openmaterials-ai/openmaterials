...
state (4 quantities, 3 operators, the first contribution to land through the
protocol gates), and a first materials-diffusion subgraph; the unified map of
55 quantities and 52 operators is content-addressed in the log-first store
(Part~\ref{part:kernel}), grown past its frozen genesis version through the
contribution gates, and the whole is held together by a suite of 875
passing tests.
...
The map is a curated reference graph. Today it holds 55 nodes (52 observable
and hidden quantities plus 3 promoted parameters) and 137 edges. Nodes are
...
the protocol. The map is small enough today (55 nodes, 52 deduplicated edges) to
...
later propose. On the current unified map the gate classifies the 52 edges as
\textbf{21 ok}, \textbf{2 documented-schematic violations}, and \textbf{29
...
operators), proposed by the sync tool and admitted through all six gates. Both
...
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  ...
  \item \textbf{Further domains through the same gates}: the deferred
        ground-state quantities (ChargeDensity, Wavefunctions, dvscf), the
        finite-difference Forces $\to$ force-constants route, and the
        \texttt{mat-*} bulk encode.
\end{itemize}
...
\texttt{b498ec02daa41c7fc8a885fd5231a8343d07e449a1f42b67aa6ff0794c4c0d64}
(2026-07-08, 108 records), eight change records past the frozen genesis hash
\texttt{e6e8044e92039696417b53b220b0f3f10559a286b0eaabbe7ea4167ff510f6cd}
(2026-07-07, the untouched 100-record prefix). The live numbers:

\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 55 nodes (52 observable and hidden quantities plus 3
        promoted parameters), 137 rendered links, 52 deduplicated operators. The
  ...
  \item \textbf{Instances.} 11 recorded values, 9 simulation and 2 measurement.
        The two newest are the first Quantum ESPRESSO values from the Si
  ...
  \item \textbf{Dimensional gate.} Of the 52 edges, 21 are ok, 2 are
        documented-schematic violations (\texttt{compute\_gruneisen} and
        \texttt{compute\_phase\_space\_3phonon}, kept schematic by choice), and 29
        ...
        \texttt{compute\_stress\_cell}) are proven ok.
  \item \textbf{Tests.} The suite is 875 passing.
\end{itemize}
