...
91 quantities and 90 operators is content-addressed in the log-first store
(Part~\ref{part:kernel}), grown past its frozen genesis version through the
contribution gates, and the whole is held together by a suite of 1237
passing tests.
...
The map is a curated reference graph. Today it holds 91 nodes (88 observable
and hidden quantities plus 3 promoted parameters) and 217 edges. Nodes are
...
Today's map is eighty-two change records past v1, its frozen genesis version:
88 typed quantities plus 3 promoted
...
mp-api 13, QE 13, LAMMPS 12, VASP 9, GPUMD 8, pycalphad 6, and a further fifteen
codes and mat-* / chem-* skills at four or fewer each), and real instances
...
\texttt{e2745ab18af17c38e4e3e26b8fba678786ad1986a27e14564c715bf0edc1bb6c}
(2026-07-10, 182 records), eighty-two change records past the frozen genesis hash
\texttt{e6e8044e92039696417b53b220b0f3f10559a286b0eaabbe7ea4167ff510f6cd}
(2026-07-07, the untouched 100-record prefix). The live numbers:

\begin{itemize}[leftmargin=2em, itemsep=0.25em]
  \item \textbf{Structure.} 91 nodes (88 observable and hidden quantities plus 3
        promoted parameters), 217 rendered links, 90 deduplicated operators,
        rendered in fourteen physics tiers (Sources, Harmonic, Thermodynamics,
        ...
        transport, Diffusion). The five newest tiers (Stability,
        Thermochemistry, Quasi-harmonic, Molecular, Electronic transport) landed
        from the AtomisticSkills scans. The
  ...
  \item \textbf{Representations.} 27 codes mapped, with per-code variable coverage:
        ...
        Quantum ESPRESSO 13, LAMMPS 12, VASP 9, GPUMD 8, pycalphad 6, matgl 5,
        mat-elasticity 5, amset 5, fairchem 4, MACE 4, OpenMM 4, ORCA 4,
        pymatgen-analysis-diffusion 4, ASE 3, mat-diffusion-analysis 2, smol 2,
  ...
  \item \textbf{Dimensional gate.} Of the 90 edges, 28 are ok, 2 are
        documented-schematic violations (\texttt{compute\_gruneisen} and
        \texttt{compute\_phase\_space\_3phonon}, kept schematic by choice), and 60
        are skipped (not closed-form \texttt{Eq} edges the gate can check, most
        of them the opaque solver edges of the newer domains). The
...
OpenMM                       & 4  & MIT \\
ORCA                         & 4  & Proprietary \\
pymatgen-analysis-diffusion  & 4  & BSD-3-Clause \\
ASE                          & 3  & LGPL-2.1 \\
...
\begin{enumerate}[leftmargin=2em, itemsep=0.2em]
  ...
  \item \textbf{Further domains and the \texttt{mat-*} bulk encode}, growing the
        ...
        instances); the AtomisticSkills scans then grew the map to record 182
        ...
        CALPHAD, amset, ORCA, and OpenMM rails.
