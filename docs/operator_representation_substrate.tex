...
The operator DAG has 19 nodes and 18 edges. Two source nodes (\texttt{Potential},
...
(12 nodes / 14 edges shown; the full DAG has 19 nodes / 18 edges once the
derived-observable tier --- PhononDOS, Grüneisen, PhaseSpace, VolumetricHeatCapacity,
MolarHeatCapacity --- is included; see \texttt{docs/dag.html} for the
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      \item \texttt{operator/nodes.py}: the 19 states as \texttt{Observable} or
            \texttt{HiddenState} instances with their fields.
      \item \texttt{operator/edges.py}: the 18 operations with sympy formulas
