...
38-node, 38-edge DAG with sympy formulas on each edge, gauge actions).
...
The operator DAG has 38 nodes and 38 edges. Two source nodes (\texttt{Potential},
...
(12 nodes / 14 edges shown; the full DAG has 38 nodes / 38 edges once the
...
Linewidth split, the Wigner/QHGK transport variants, and the cumulative-$\kappa$
distributions --- is included; see \texttt{docs/dag.html} for the
interactive view). The
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      \item \texttt{operator/nodes.py}: the 38 states as \texttt{Observable} or
            \texttt{HiddenState} instances with their fields.
      \item \texttt{operator/edges.py}: the 38 operations with sympy formulas
