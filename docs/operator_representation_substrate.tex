...
43-node, 44-edge DAG with sympy formulas on each edge, gauge actions).
...
The operator DAG has 43 nodes and 44 edges. Two source nodes (\texttt{Potential},
...
\texttt{VelocityAutocorrelation}, and \texttt{MeanSquaredDisplacement}. These feed
LAMMPS and GPUMD adapters in Phase 2; the MD-based $\kappa$ contractions
(Green-Kubo, NEMD, HNEMD) land in P3.
...
(12 nodes / 14 edges shown; the full DAG has 43 nodes / 44 edges once the
...
distributions, and the MD-primitive tier (Trajectory, HeatCurrent, HeatCurrentACF,
VelocityAutocorrelation, MeanSquaredDisplacement) --- is included; see
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      \item \texttt{operator/nodes.py}: the 43 states as \texttt{Observable} or
            \texttt{HiddenState} instances with their fields.
      \item \texttt{operator/edges.py}: the 44 operations with sympy formulas
