...
\texttt{omai.representation} (the \texttt{StateRepresentationSpec} and
\texttt{OperationRepresentationSpec} types, \texttt{Unit} and unit conversions,
...
pair of \texttt{StateRepresentationSpec}s verifies that two codes are claiming
...
field on one state:
\begin{equation*}
  m = (\textsc{StateRepresentationSpec}, \texttt{field\_name}, x)
\end{equation*}
where \textsc{StateRepresentationSpec} carries the operator state $s$, the representation name, and
...
\paragraph{Why the representation spec ships with the data.} Different codes
...
the operator layer computes the cross-representation conversion factor mechanically and surfaces
...
representation-to-representation primitive. The primitives are the two functions
$\texttt{representation\_to\_operator}(A,\text{obs})$ and
$\texttt{operator\_to\_representation}(B,\text{obs})$ which carry a
value between a representation's emitted form and the operator form
($x_{\text{operator}} = x_A \cdot u_A^{\text{canon}} / c_A$, where
$u_A^{\text{canon}}$ is the unit's \texttt{to\_canonical} multiplier and
$c_A$ is the representation's convention deviation from operator-canonical).
The combined factor
...
\end{equation*}
which takes two representations of the same state and field, applies the cross-representation
...
(\texttt{StateRepresentationSpec}), plus per-operation parameter units, algorithmic-convention
overrides, and discretization choices (\texttt{OperationRepresentationSpec}). Differences
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.representation} --- the bridge.
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item \texttt{adapter.py}: \texttt{StateRepresentationSpec} and
            \texttt{OperationRepresentationSpec}, plus inter-representation factor
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item \texttt{representation/kaldo.py},
            ...
            carries an \texttt{OperationRepresentationSpec}; trivial source /
