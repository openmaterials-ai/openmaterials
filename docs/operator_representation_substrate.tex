...
\texttt{docs/index.html} for the interactive view). The
...
Each adapter is a representation functor over the operator DAG. Seven adapters
are now ingested, in two tiers. The four BTE / harmonic codes that consume
force constants:
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{LAMMPS} (C++/Python). Classical molecular dynamics. Native
        \texttt{pair\_style} Potential, plus the MD-primitive tier
        (\texttt{Trajectory}, \texttt{HeatCurrent}, \texttt{HeatCurrentACF},
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item \texttt{visualize.py}: emits \texttt{docs/index.html}, an interactive
