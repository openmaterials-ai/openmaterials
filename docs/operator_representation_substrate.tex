...
The operator DAG has 14 nodes and 13 edges. Two source nodes (\texttt{Potential},
...
\caption{The operator DAG for lattice thermal transport: 14 nodes and 13 edges. The
...
Each adapter is a representation functor over the operator DAG. Phase 1 supplies two:
  ...
  \item \texttt{omai.representation} --- the bridge.
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item \texttt{compare.py}: \texttt{compare()} with the five-status
            \texttt{RepresentationComparisonResult}.
    \end{itemize}
    Code-agnostic.
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      \item \texttt{operator/nodes.py}: the 14 states as \texttt{Observable} or
            \texttt{HiddenState} instances with their fields.
      \item \texttt{operator/edges.py}: the 13 operations with sympy formulas;
      ...
      \item \texttt{represented/kaldo.py}, \texttt{represented/phono3py.py}:
