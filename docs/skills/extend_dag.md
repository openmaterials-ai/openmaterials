...
states, no cycles) and does not pollute downstream consumers.
...
### Pattern A — type-level parameter on the state

When the variants change **gauge type** (Observable vs HiddenState) and the
parameterised state is **terminal** (or its parameterised consumers are
themselves a closed sub-branch).

Examples in the repo:
- `MeanFreeDisplacement[bte_solver=rta]` (HiddenState) vs `[direct_inverse]`
...
**Anti-pattern.** Putting a type parameter on an *intermediate* state. Every
...
forced `compute_dispersion`, `compute_group_velocity`, `compute_linewidth`,
`solve_bte_rta`, `solve_bte_direct`, and `contract_kappa_*` to all gain the
parameter.

### Pattern B — sibling states, converging edge
...
parameter variants of one production formula. Sibling states make the
...
in-place modifier — one state with an edge that both consumes and produces
...
1. **Is it a new physical quantity** (not a variant)? Add a new state node;
   add one or more edges producing it; declare gauge type, dimension,
   conventions, indices.

2. **Is it a variant that changes the gauge type** (Observable ↔
   HiddenState)? Pattern A — type-level parameter on the (terminal) state.
...
1. **Operator layer.**
   - `omai/<domain>/operator/nodes.py`: declare new `State` instances.
     Pattern A states use a `type_parameters` dict; Pattern B / C states
     have plain names. Set `kind="observable"` or `"scaffolding"`,
     `gauge_group`, `canonical_conventions`, `convention_factors`,
     `fields`, indices.
   - `omai/<domain>/operator/edges.py`: declare new `Operation` instances.
     Each edge carries a sympy `formula` (use `auxiliary_formulas` when a
     kernel reappears in two edges — see `compute_linewidth` /
     `solve_bte_direct` for the |V₃|² pattern). Declare
     `algorithmic_conventions` only for choices a downstream comparison
     needs to distinguish.
   - `omai/<domain>/operator/__init__.py`: re-export.

2. **Adapters.** For every code that produces the new quantity, add a
   `StateAdapterSpec` and (where the operation has algorithmic
   conventions) an `OperationAdapterSpec` in
...
3. **Tests.** Add a smoke test in `tests/test_operator.py` asserting the
   node and edge identities, the inputs/outputs, and any new algorithmic
   conventions. If the new edge has a non-trivial formula, assert that
   `formula.free_symbols` contains the expected ingredients.

4. **Visualization.** `docs/dag.html` regenerates from
   `python -m omai.thermal_transport.visualize docs/dag.html` (or the
   per-domain equivalent). No code changes are needed unless you are
   adding a new pattern that the renderer cannot lay out automatically.
...
- **Edge formulas that re-derive an existing kernel.** If a sub-expression
  ...
  declared once on `compute_linewidth` and referenced from
  `solve_bte_direct`'s collision matrix.
- **States with no clear gauge type.** Every State must declare whether it
  is gauge-invariant (Observable) or gauge-dependent (HiddenState).
  HiddenStates must declare their `gauge_group` and the cross-code
