...
`index.json` in this directory is derived data, a byte-stable projection of the
target files (regenerated by `python -m omai.map_data`, guarded by the suite):
the playground datasheets read it to list, under "Reproduce this", the pinned
runs for a record's node. The target files stay the source of truth.
