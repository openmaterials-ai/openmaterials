...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.4em]
  ...
  \item \textbf{Abstract states are unit-free.} Dimensionful quantities (frequency,
        energy, length, time) exist at the abstract layer, but specific unit choices
        (THz vs eV vs rad/s, \AA{} vs Bohr) do not. Unit choice belongs to the
        materialization and is declared by the adapter that produced it; the substrate
  ...
  \item \textbf{The atom is a symbolic operation.} The atomic unit of meaning is a typed
        transformation between abstract states, not an instruction in an input file or an
        ...
        what it computes. The formula is the substrate's claim, machine-readable and
        Lean-projectable, against which adapter conformance can be checked.

  \item \textbf{States are typed witnesses; equivalently, observables are nodes.} A state
        is the typed claim that an abstract physics object exists with a particular
        history---not the data the object would have when computed. In practice each
        state corresponds to a single observable, so the substrate uses ``state'',
        ``observable'', and ``node'' interchangeably. Each observable declares an index
        signature (e.g.\ $\omega(\mathbf{q},\nu)$ has indices $(\mathbf{q}, \nu)$;
  ...
  \item \textbf{Operation identity is parameterized.} An operation is identified by its
        ...
        deviation vs.\ by half-width) are canonicalized at the abstract layer to a single
  ...
  \item \textbf{Cross-code agreement is at the observable layer.} Two materializations
        of the same abstract state produced by different adapters need not agree
        numerically at intermediate decompositions (per-mode linewidths, basis-dependent
        intermediate sums); they must agree at extracted observables. Each abstract
        observable carries a comparison protocol declaring its expected tolerance
        across adapters. Contracted scalars and tensors (e.g., $\kappa^{\alpha\beta}$,
        $\Sigma\Gamma_{q\nu}$) carry tight protocols; per-mode arrays whose values
        depend on summation order or symmetry-reduction choices carry loose or
        not-comparable protocols.
  ...
  \item \textbf{Sources are produced by nullary operations.} Source states (the
        ...
        substrate has no special ``input'' category. The materialization of a source
...
An abstract state $s$ is unit-free: its physics type carries the dimension of any
...
An abstract state $s$ is a tuple $(\tau, \pi, \mathcal{I})$ where:
\begin{itemize}[leftmargin=2em]
  \item $\tau$ is a \emph{physics type} drawn from a finite registry: derived types
        include \texttt{ForceConstants[order=$n$]}, \texttt{DynamicalMatrix},
        \texttt{Frequency}, \texttt{Eigenvectors}, \texttt{GroupVelocity},
        \texttt{HeatCapacity}, \texttt{Linewidth}, \texttt{MeanFreeDisplacement},
        \texttt{ThermalConductivity}; source types include \texttt{Potential} (the
        abstract root, stubbed in Phase 1), \texttt{Temperature},
        \texttt{LatticeConstant}, \texttt{Pressure}, and measured-observable types;
  ...
  \item $\mathcal{I}$ is the \emph{index signature}: the tuple of named indices the
        observable carries (e.g., $\omega$ has $\mathcal{I} = (\mathbf{q}, \nu)$;
        $v$ has $(\alpha, \mathbf{q}, \nu)$; $\kappa$ has $(\alpha, \beta)$).
        Empty for scalar / opaque observables.
\end{itemize}
Two abstract states are equal iff $\tau_1 = \tau_2$ and $\pi_1 = \pi_2$. The composed
approximation error on $s$ (a Phase 2 capability) would be
\begin{equation}
  \epsilon_{\text{approx}}(s) = \mathrm{compose}\bigl(\epsilon_{\mathcal{O}_1},\, \epsilon_{\mathcal{O}_2},\, \ldots,\, \epsilon_{\mathcal{O}_n}\bigr).
\end{equation}

\paragraph{Source versus derived states.} Abstract states fall into two kinds, both
...
\begin{itemize}[leftmargin=2em]
  \item \emph{Source states} are the outputs of \emph{nullary} operations
        ...
        with no abstract-state inputs. They are empirical inputs (lattice constant,
        temperature, pressure, applied field), the abstract \texttt{Potential} itself
...
observables (``how does a measured $\kappa(T)$ enter the substrate?''), and the unmodeled
abstract root in Phase 1 (``what is the silicon potential, before we differentiate
...
\begin{itemize}[leftmargin=2em]
  \item a \emph{symbolic formula} $\mathcal{F}_\mathcal{O}$ stating what the operation
        ...
        implicit ones (eigenvalue equations, linear systems). Source operations
        (\texttt{provide\_*}) carry $\mathcal{F}_\mathcal{O} = \emptyset$;
  ...
  \item a possibly empty set of \emph{algorithmic conventions} (per
        ...
        (dimensioned but unit-free at the abstract layer).
\end{itemize}
...
$\mathcal{F}_\mathcal{O}$ is the substrate's machine-readable claim about what the
...
$(s, \Sigma, x, U, \epsilon_{\text{disc}})$ where:
\begin{itemize}[leftmargin=2em]
  ...
  \item $U$ is the \emph{unit declaration}: a mapping from each dimensionful quantity in $\tau$
        to the unit in which it is expressed in $x$ (e.g., for \texttt{Dispersion},
        \{\texttt{frequency}: linear THz, \texttt{group\_velocity}: \AA{}$\cdot$THz\}). The
        adapter that produced $m$ supplies $U$;
...
\paragraph{Why $U$ is part of the materialization.} Different codes express the same
physics in different units (e.g., kaldo's linewidth is in angular-frequency THz while
phono3py's is in linear-frequency THz, a factor of $4\pi$; kaldo's heat capacity is in
J/K, phono3py's in eV/K, a factor of $e$). A materialization without $U$ is unsafe to
compare against another; with $U$, the substrate can convert both to a canonical unit
before any comparison and surface unit mismatches as adapter conformance failures rather
than as numerical mysteries.

\subsection{Observable extraction and comparison protocols}

Cross-adapter agreement is checked at the observable layer, not at the raw materialization
layer. An \emph{observable} is a pure function $\mathcal{Q} : x \mapsto y$ that extracts a
physically meaningful quantity from the numerical content $x$ of a materialization (a
contraction, a peak position, an integrated quantity). Each abstract state declares its
observables; each observable carries a \emph{comparison protocol} specifying how two
extractions $y_1$, $y_2$ from materializations of the same abstract state by different
adapters are compared---typically a tolerance, but possibly a richer relation (e.g., ``equal
up to a permutation of degenerate modes'' or ``not directly comparable, contraction
required'').

The protocol records the substrate's claim about which extractions are stable across
adapter choices and which are not. A contracted scalar like $\kappa^{\alpha\beta}$ or
$\Sigma\Gamma_{q\nu}$ has a tight tolerance: disagreement is a real discrepancy that must
be diagnosed. A per-mode array whose values depend on the summation order in the
underlying sum (e.g., per-mode $\Gamma_{q\nu}$ in a Brillouin-zone integration that one
adapter handles by symmetry-reduced triplets and another handles by enumeration) is
explicitly marked as not-directly-comparable: numerical disagreement at the mode level is
expected and not a substrate failure, even though the contracted total agrees.

\paragraph{Why this is the right level.} Two adapters that disagree at the observable
layer disagree about physics. Two adapters that disagree at the raw-materialization
layer but agree at the observable layer disagree only about bookkeeping---the order in
which a sum is decomposed, the basis in which a tensor is written. The substrate's job
is to make this distinction structural: by declaring which observables are
adapter-invariant, the substrate states what it is asking for and what it is not.
...
The abstract DAG has 12 nodes (states / observables) and 11 edges (operations). Two
nodes (\texttt{Potential}, \texttt{Temperature}) are sources, produced by nullary
\texttt{provide\_*} operations.

\paragraph{Nodes.}
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  \item \texttt{Potential}: the abstract root, materialized in Phase 1 as a stubbed
        source state (the silicon potential, the LAMMPS-Tersoff potential, or the QE-PBE
        potential, as an opaque label);
  \item \texttt{Temperature}: source state, scalar $T$;
  \item \texttt{ForceConstants[order=2]}, \texttt{ForceConstants[order=3]}: harmonic and
        cubic interatomic force constants $\Phi^{(n)}$;
  \item \texttt{DynamicalMatrix}: the Bloch sum $D_{ij}(\mathbf{q})$;
  \item \texttt{Frequency}: $\omega(\mathbf{q},\nu)$, indices $(\mathbf{q},\nu)$;
  \item \texttt{Eigenvectors}: $e(\mathbf{q},\nu)$, indices $(i, \mathbf{q},\nu)$;
  \item \texttt{GroupVelocity}: $v^\alpha(\mathbf{q},\nu)$, indices
        $(\alpha, \mathbf{q}, \nu)$;
  \item \texttt{HeatCapacity}: $c(\mathbf{q},\nu, T)$, indices $(\mathbf{q}, \nu)$;
  \item \texttt{Linewidth}: $\Gamma(\mathbf{q},\nu, T)$, indices $(\mathbf{q}, \nu)$;
  \item \texttt{MeanFreeDisplacement}: $F^\alpha(\mathbf{q},\nu, T)$, indices
        $(\alpha, \mathbf{q}, \nu)$;
  \item \texttt{ThermalConductivity}: $\kappa^{\alpha\beta}(T)$, indices
        $(\alpha, \beta)$.
\end{itemize}

\paragraph{Edges.} Verb-headed names; each carries a sympy formula:
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  \item \texttt{provide\_potential}: $() \to$ \texttt{Potential};
  \item \texttt{provide\_temperature}: $() \to$ \texttt{Temperature};
  \item \texttt{compute\_force\_constants[order=2]}, \texttt{[order=3]}:
        \texttt{Potential} $\to$ \texttt{ForceConstants[order=$n$]} (stubbed in Phase~1);
  \item \texttt{compute\_dynamical\_matrix}: \texttt{ForceConstants[order=2]} $\to$
        \texttt{DynamicalMatrix};
  \item \texttt{compute\_dispersion}: \texttt{DynamicalMatrix} $\to$
        (\texttt{Frequency}, \texttt{Eigenvectors})  [multi-output, implicit equation];
  \item \texttt{compute\_group\_velocity}: (\texttt{DynamicalMatrix},
        \texttt{Frequency}, \texttt{Eigenvectors}) $\to$ \texttt{GroupVelocity};
  \item \texttt{compute\_heat\_capacity}: (\texttt{Frequency}, \texttt{Temperature})
        $\to$ \texttt{HeatCapacity};
  \item \texttt{compute\_linewidth}: (\texttt{Frequency}, \texttt{Eigenvectors},
        \texttt{ForceConstants[order=3]}, \texttt{Temperature}) $\to$ \texttt{Linewidth},
        with parameter $\sigma$ (broadening) and algorithmic convention
        \texttt{broadening\_param};
  \item \texttt{solve\_bte}: (\texttt{Frequency}, \texttt{GroupVelocity},
        \texttt{Linewidth}, \texttt{Temperature}) $\to$ \texttt{MeanFreeDisplacement},
        with algorithmic convention \texttt{bte\_solver} $\in$ \{rta, iterative,
        direct\_inverse\};
  \item \texttt{contract\_kappa}: (\texttt{HeatCapacity}, \texttt{GroupVelocity},
        \texttt{MeanFreeDisplacement}) $\to$ \texttt{ThermalConductivity}.
\end{itemize}
...
adapter-level steps recorded as provenance, not modeled by the substrate.
...
% BTE
\node[abstractnode] (MFD) at (-1.5, -7.0) {MeanFreeDisplacement};

% kappa
\node[abstractnode] (KAP) at ( 4, -8.4) {ThermalConductivity};
...
\draw[arr] (TMP) -- (HC);
\draw[arr] (FRQ.south) to[bend right=30] node[oplabel, pos=0.6] {} (MFD.west);
\draw[arr] (GV) -- (MFD);
\draw[arr] (LW) -- node[oplabel] {solve\_bte} (MFD);
\draw[arr] (TMP.south) to[bend left=30] node[oplabel] {} (MFD.east);
\draw[arr] (HC.south) to[bend left=15] node[oplabel] {contract\_kappa} (KAP.east);
\draw[arr] (GV.south) to[bend right=30] node[oplabel, pos=0.6] {} (KAP.west);
\draw[arr] (MFD) -- (KAP);

\end{tikzpicture}%
}
\caption{The abstract DAG for lattice thermal transport: 12 nodes (states / observables)
and 11 edges (operations). The two darker boxes (\texttt{Potential},
\texttt{Temperature}) are source nodes, produced by nullary \texttt{provide\_*}
operations. Dashed edges from \texttt{Potential} to \texttt{ForceConstants} represent
upstream operations declared in the substrate but stubbed in Phase 1. Each remaining
edge carries a symbolic formula (sympy expression or sympy.Eq). The DAG is uniform: every
node is the output of some operation; cross-code comparison reduces to graph alignment
over this template plus per-state observable extraction. Adapter-specific
materializations attach to each node and are not shown for clarity.}
\label{fig:dag}
\end{figure}
...
The Python package \texttt{omai} factors the substrate into three top-level
sub-packages plus one per domain:

\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  \item \texttt{omai.abstract}: substrate machinery for the abstract world.
        \texttt{State}, \texttt{Operation}, \texttt{Observable}, \texttt{Parameter},
        \texttt{Dimension} (class), \texttt{PhysicsType} (closed enum), and
        \texttt{topological\_order}. Code-agnostic and domain-agnostic.
  \item \texttt{omai.materialization}: the bridge. \texttt{Unit} class with the unit
        registry plus strict same-dimension \texttt{conversion\_factor};
        \texttt{StateAdapterSpec} and \texttt{OperationAdapterSpec} with cross-state /
        cross-operation comparison helpers. Code-agnostic.
  \item \texttt{omai.thermal\_transport}: the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      \item \texttt{symbolic.py}: the 12 nodes and 11 edges of \S\ref{sec:demo} as
            \texttt{State} and \texttt{Operation} instances, with sympy formulas on
            every derived edge.
      \item \texttt{materialized/kaldo.py}, \texttt{materialized/phono3py.py}:
            per-code adapter specs (one file per code), each declaring units,
            conventions, and discretization choices for the abstract DAG.
    \end{itemize}
\end{itemize}

\noindent The split mirrors the substrate's architectural commitment: \texttt{abstract}
and \texttt{materialization} are the substrate's two worlds plus its bridge;
\texttt{thermal\_transport} is the first domain instantiated against them. Future
domains (electronic transport, optical response, etc.) follow the same two-file pattern
in their own folder; no change to \texttt{abstract} or \texttt{materialization} is
required to add one.
