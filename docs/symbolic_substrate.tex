...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.4em]
  ...
  \item \textbf{States are typed witnesses; nodes are Observables or HiddenStates.}
        A state is the typed claim that an abstract physics object exists with a
        particular history. The substrate splits states into two structural kinds:
        ...
        substrate does not pin down). Each state declares fields, and each field has an
  ...
  \item \textbf{Cross-code agreement is required at Observable nodes only.} Two
        ...
        the substrate's \texttt{compare} returns the status \texttt{NOT\_COMPARABLE}
...
An abstract state $s$ is a tuple $(\kappa, \tau, \pi, F)$ where:
\begin{itemize}[leftmargin=2em]
  \item $\kappa \in \{\textsc{Observable}, \textsc{HiddenState}\}$ is the
        ...
        choice, degenerate-subspace rotation, …) that the substrate doesn't pin down,
...
\end{itemize}
Two abstract states are equal iff their names match. The composed approximation error
...
\end{equation*}
where \textsc{StateAdapterSpec} carries the abstract state $s$, the adapter name, and
...
the substrate computes the cross-adapter conversion factor mechanically and surfaces
...
\begin{description}[leftmargin=2.5em, itemsep=0.2em]
  \item[\textsc{Expected\_Pass}] the state is an Observable; the arrays agree within
        $(\text{rtol}, \text{atol})$. The substrate's prediction holds.
  ...
  \item[\textsc{Not\_Comparable}] the state is a HiddenState and no contraction was
        supplied. The substrate refuses to make a pass/fail verdict; residuals are
...
The status taxonomy separates three things that earlier versions of the substrate
conflated: the substrate's \emph{prediction} (would these agree?), the empirical
...
The Observable / HiddenState distinction is load-bearing, so the substrate
...
A \texttt{validate\_substrate} function walks the node and edge sets at
...
reference unknown nodes. The thermal-transport substrate passes this
validator; tests check it on every CI run.

\subsubsection*{Symbolic invariance via \texttt{GaugeAction}}

Beyond named declarations, the substrate supports \emph{symbolic proofs}
...
an operation's sympy formula, the substrate substitutes the gauge action
...
\end{equation*}
gives $e^{-i\theta_{q,\nu}} \cdot e^{i\theta_{q,\nu}} = 1$, and sympy
mechanically reduces the difference to zero. This is the substrate's
...
the substrate falls back to the Level 1 named-gauge declarations.

\paragraph{Crystal symmetry as data-driven gauges.} The substrate does not
hardcode any specific point group. Crystal symmetry enters as \emph{data}:
a \texttt{SymmetryOperation} (3$\times$3 integer rotation matrix plus
translation, matching spglib's data model) and a \texttt{CrystalPointGroup}
(a list of generators). Materials adapters that use spglib internally
translate its output into these substrate-level types; adapters with
their own analyzer do the same. The substrate is spglib-agnostic but
spglib-compatible.

Given a \texttt{SymmetryOperation}, the factory
\texttt{fc2\_gauge\_from\_symmetry\_op} builds a \texttt{GaugeAction} for
its action on $\Phi^{(2)}$ if the rotation is substitution-friendly
(diagonal in Cartesian indices with $\pm 1$ entries — identity, inversion,
Cartesian-aligned mirrors with uniform signs). For these, the substrate
mechanically verifies invariance via sympy. For non-diagonal rotations
(e.g., 90$^\circ$ around $z$), the factory returns None: the gauge is
declared at Level 1 but not symbolically verified --- substrate machinery
to expand the FC$^{(2)}$ index space and prove invariance under coupled
rotations is research-grade.

The order-2 inversion group $C_i$ is verified end-to-end: the
$Z/2$-symmetrized $\Phi^{(2)}_\text{sym}(\mathbf{R}) = \tfrac{1}{2}
(\Phi^{(2)}(\mathbf{R}) + \Phi^{(2)}(-\mathbf{R}))$ is provably invariant;
the bare un-symmetrized $\Phi^{(2)}$ is correctly flagged as not invariant.
Other crystal point groups are constructed by the same data path; the
substrate-level interface is the same.
...
The abstract DAG has 14 nodes and 13 edges. Two source nodes (\texttt{Potential},
...
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  \item \texttt{Potential} (\textbf{O}): the abstract root, stubbed in Phase 1;
...
\caption{The abstract DAG for lattice thermal transport: 14 nodes and 13 edges. The
...
nodes (after the substrate's spec-derived unit and convention conversion). Other
...
Each adapter is a materialization functor over the abstract DAG. Phase 1 supplies two:
...
from the substrate's canonical declarations surface as conversion factors at
spec-load time.

\subsection{Verification on silicon (Tersoff potential)}
\label{ssec:silicon-verification}

The substrate's symbolic predictions have been verified end-to-end against numerical
...
\begin{tabular}{lll}
\toprule
Observable / contraction & Substrate prediction & Empirical residual \\
\midrule
...
Both factors were derived by the substrate from declared adapter specs, then
applied mechanically to the diagnostic data.

The 4\% disagreement on $\kappa_\text{RTA}$ is the substrate's structural prediction
...
The Python package \texttt{omai} factors the substrate into two substrate sub-packages
plus one per domain:
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  \item \texttt{omai.abstract} --- substrate machinery for the abstract world.
        ...
        \texttt{validate\_substrate} (Level 1 discipline enforcement), and
  ...
  \item \texttt{omai.thermal\_transport} --- the lattice-thermal-transport domain.
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item \texttt{visualize.py}: emits \texttt{docs/dag.html}, an interactive
            ...
            panels, MathJax-rendered formulas) reflecting the current substrate.
    \end{itemize}
\end{itemize}

\noindent The split mirrors the substrate's architectural commitment: \texttt{abstract}
is the abstract world; \texttt{materialization} is the bridge to the numeric world;
...
own folder; no change to the substrate-level packages is required to add one.
