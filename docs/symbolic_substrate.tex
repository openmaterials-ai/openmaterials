...
\begin{enumerate}[leftmargin=2.5em, itemsep=0.4em]
  ...
  \item \textbf{Identity is type plus provenance.} An symbolic state is identified by its
...
An symbolic state $s$ is unit-free: its physics type carries the dimension of any
...
An symbolic state $s$ is a tuple $(\kappa, \tau, \pi, F)$ where:
\begin{itemize}[leftmargin=2em]
...
An symbolic workflow is a finite directed acyclic graph $W = (V, E)$ where $V$ is a set of
