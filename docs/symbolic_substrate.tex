...
\paragraph{Crystal symmetry as an abstract declaration.} Crystal symmetry
at the substrate level is purely a \emph{declaration}: a
...
group-element enumeration --- lives in the substrate.
...
ShengBTE, \ldots), typically via spglib. The substrate's role is just to
...
This is the same pattern \texttt{Potential} follows: declared abstractly
as a labelled source state without the substrate modeling its functional
form. ``How $\Phi^{(2)}$ is computed under crystal symmetry'' is a code
concern; ``which symmetry group was assumed'' is a substrate-level
declaration.
